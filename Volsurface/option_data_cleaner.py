...
def extract_options_data(excel_file_path):
    ...
    spot_price_map = dict(zip(
        pd.to_datetime(price_df['Date']).dt.date,
        price_df['Close\xa0']  # Note: 'Close' column has non-breaking space
    ))

    # Extract relevant columns from option sheet
    data = {
        'option_contract': option_df['Contract Name'],
        'trading_date_raw': option_df['Last Trade Date (EST)'],
        'option_price': option_df['Last Price'],

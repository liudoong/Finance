...
class VolatilitySurface:
    ...
    def _build_interpolator(self):
        ...
        if len(points) > 0:
            # Use smoothing parameter to reduce overfitting
            # smoothing=0.0 → exact interpolation (bumpy)
            # smoothing>0 → smoothed approximation (recommended: 0.001-0.01)
            self.interpolator = RBFInterpolator(
                np.array(points),
                np.array(values),
                kernel='thin_plate_spline',
                smoothing=0.005  # Small smoothing to reduce bumps
            )
        else:
